...
class Wattpad:
    ...
    def get_book_by_url(self, url) -> Book:
        ...
        # Get book stats
        stats: [BeautifulSoup] = soup.find(
            class_='new-story-stats')  # type: ignore
        if stats is None:
        ...
        toc: [BeautifulSoup] = soup.find(
            class_='table-of-contents')  # type: ignore
        if toc is None:
    ...
    # type: ignore
    def search_books(self, query: str, limit: int = 15, start: int = 0, mature: bool = True, free: bool = True, paid: bool = True, completed: bool = False, show_only_total: bool = False) -> List[Book]:
        """
        Args:
        ...
        # options
        self.log.debug(
            f"Options: start: {start} limit: {limit} mature: {mature} free: {free} paid: {paid} completed: {completed}")
    ...
    def get_user_reading_lists(self, start=0, limit=100, username=None) -> List[ReadingList]:
        ...
        # type: ignore
        return request.get_user_reading_lists(start, limit, username)
